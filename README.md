# 🚗 SWP391_AutoWashPro - Automated Car Wash Management System
...
│   ├── app/
│   │   ├── core/                 # Config, security (JWT/bcrypt), dependencies (M3)
│   │   ├── db/                   # SQLAlchemy engine & session (M5)
...
│   └── requirement.txt          # Các thư viện Python bắt buộc
│
...
> ⚠️ **Lưu ý:** Codebase hiện tại (`app/models/*.py`, `app/core/config.py`, `requirement.txt`) đang được viết cho **PostgreSQL** (kiểu `UUID` của Postgres, driver `psycopg2-binary`). Nếu team quyết định dùng **MySQL** thay vì Postgres, cần cập nhật lại các model (`UUID` → `CHAR(36)`), driver kết nối (`pymysql`), và `DATABASE_URL` trước khi dùng README này để cài đặt.
...
Tạo file `.env` trong thư mục `backend/` (không commit file này lên Git) dựa theo các biến khai báo trong `app/core/config.py`, ví dụ:

```
DATABASE_URL=postgresql://postgres:password@localhost:5432/autowash
...
Sau khi chạy, truy cập `http://127.0.0.1:8000/docs` để xem tài liệu tương tác API tự động (Swagger UI).
